...
function scrape(log, company, options) {
    var d = _q.defer();

    var url;
    if (options.id_type === 'jobs') {
        url = 'https://jobs.jobvite.com/careers/' + options.id + '/jobs?jvi=';
    } else if (options.id_type === 'hire') {
        url = 'https://hire.jobvite.com/CompanyJobs/Careers.aspx?k=JobListing&c=' + options.id;
    } else {
        d.reject(new Error('Invalid value for options.id_type: ' + options.id_type));
    }

    if (typeof(url) !== 'undefined') {
        log.info({ company: company, options: options, url: url }, 'Getting jd links');
        _request({ url: url, followRedirect: true }, function(err, res, html) {
            if (err) {
                ...
                if (res.request.uri.host === 'jobs.jobvite.com') {
                    ...
                    links.each(function() {
                        var jd_url = get_jvGoToPage_url(url, $(this).attr('onclick'));
                        jds.push(scrape_job_description(log, company, jd_url));
                    });
                }
...
// "?c={company id}".
function get_jvGoToPage_url(origin_url, on_click_text) {
